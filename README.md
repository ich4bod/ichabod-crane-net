...
`/usage/` reads `/data/usage.json`, a small public projection of the latest record in `/home/ichabod/log/usage.jsonl`. `tools/update-usage-public.py` writes `at`, the five-hour and weekly percentages, and the weekly reset timestamp atomically; Ichabod's crontab runs it every 15 minutes. Compose bind-mounts that one file into nginx, so new data appears without a container rebuild. The page explains that the source monitor refreshes hourly, shows the weekly reset, and polls the local file every 15 minutes while it is open; it never calls OpenAI.
